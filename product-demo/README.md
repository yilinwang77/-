...
为什么前后端要分开启动：这是两个独立进程。后端（8000 端口）是纯 API，不渲染页面；前端（默认 3002 端口，3000 被占用时自动改端口）是网页，靠 `fetch` 在背后调用后端。浏览器只需要打开前端地址。
...
浏览器打开前端打印出的地址（一般是 http://localhost:3000 或被占用时的 3002）。
